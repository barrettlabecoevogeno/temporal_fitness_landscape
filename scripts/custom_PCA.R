...
custom_pca <- function(pca = pca, 
                       ...
                       zoom=1,
                       square=FALSE,
                       name.pc = c("PC1","PC2","PC3"),
                       linex = 3,
                       liney = 3,
                       square.size = 1,
                       threshold = 0, 
                       main = 'PCA',
                       pch = 4, # 21 for filled circles 
                       col.arrow = "grey",
                       col.label = "red",
                       size.label = .7,
                       col.sites = "black",
                       ...
                       vector.names = NULL,
                       font.type = 1, # 2 bold for arrow labels
  ...
  pca.axis.name = c(paste(name.pc[1]," ", "(", prop.exp[1], "%", ")", sep = ""),
                    paste(name.pc[2]," ", "(", prop.exp[2], "%", ")", sep = ""),
                    paste(name.pc[3]," ", "(", prop.exp[3], "%", ")", sep = ""))[choices]
  zoom = zoom
  ...
  if (centered) {
    ...
    myccaplot(pca,
              ...
              pch=4,
              axes = FALSE)
    axis(1, cex.axis = size.axis.labels)  ### (2)
    axis(2, cex.axis = size.axis.labels)  ### (2)
    mtext(pca.axis.name[1], side=1, line=linex, col="black", cex=size.axis.labels)
    mtext(pca.axis.name[2], side=2, line=liney, col="black", cex=size.axis.labels)
